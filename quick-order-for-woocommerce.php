...
add_action('admin_enqueue_scripts', function ($hook) {
    if ('toplevel_page_quick-order-create' == $hook) {
        wp_enqueue_style('qofw-style', plugin_dir_url(__FILE__) . 'assets/css/style.css', [], '1.0.0');
        wp_enqueue_script('qofw-script', plugin_dir_url(__FILE__) . 'assets/js/qofw.js', ['jquery', 'thickbox'], '1.0.0', true);
        ...
        wp_localize_script('qofw-script', 'qofw', [
            ...
            'pt'       => __('WooCommerce Quick Order', 'qofw'), // plugin name
        ]);
        add_thickbox();
...
function qofw_admin_page()
{
    ...
    <div class="qofw-form-wrapper">
        <div class="qofw-form-title">
            <h4>
                <?php _e('WooCommerce Quick Order', 'qofw'); ?>
            </h4>
        </div>
                    ...
                    <fieldset>
                        ...
                        <div class='pure-control-group'>
                            ...
                            <select class='qofw-control' name='item' id='item'>
                                <option value="0">
                                    <?php _e('Select One', 'qofw'); ?>
                                </option>
                                <?php
                                $products = wc_get_products(['post_status' => 'publish', 'posts_per_page' => -1]);
...
function qofw_process_submission()
{
    ...
    if (sanitize_text_field($_POST['custom_id']) == 0) {
        ...
        $customer = new WP_User(sanitize_text_field($_POST['custom_id']));
    }
